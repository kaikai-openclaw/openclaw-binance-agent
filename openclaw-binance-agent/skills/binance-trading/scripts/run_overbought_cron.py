...
脚本直接执行超买扫描和交易流水线，并输出稳定结构的 JSON 或 Markdown。
...
from src.skills.crypto_overbought import (
    LongTermOverboughtSkill,
    ShortTermOverboughtSkill,
    HourlyOverboughtSkill,
)
...
_log_dir = os.path.join(DB_DIR, "logs")
os.makedirs(_log_dir, exist_ok=True)
_log_file = os.path.join(_log_dir, "overbought_cron.log")
logging.getLogger().handlers[0].stream = open(_log_file, "a", encoding="utf-8")
...
def render_markdown(report: dict) -> str:
    ...
    lines = [
        ...
        f"- 超买候选: {scan['filter_summary'].get('after_overbought_filter', 0)}",
    ...
    for item in scan["candidates"][:3]:
        lines.append(
            "- {symbol}: {score}/100, 24h {change:+.2f}%, RSI {rsi}, 费率 {funding}, {sig}".format(
                ...
                score=item.get("overbought_score", 0),
...
def run_report(args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        scan_data = overbought_skill.run(scan_input)
