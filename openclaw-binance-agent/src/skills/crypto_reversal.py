"""
加密货币合约右侧反转筛选 Skill（三模式：4h / 1h / 1d）

与超跌反弹（crypto_oversold）的区别：
  超跌反弹 = "接飞刀"，在暴跌过程中抄底
  右侧反转 = "确认转向"，等底部构筑完成后、动量确认时入场

核心逻辑：
  前期深度下跌 → 价格从底部反弹至理想区间（3-12%）→ KDJ 超卖金叉确认动量反转
  → 资金费率从极端回归 → 放量/企稳等辅助信号加分

## 筛选流程
  1. 下架预警过滤：deliveryDate 14 天内的合约自动排除
  2. 市场广度分层：< 20% 硬阻断，20-35% 严格谨慎，35-45% 谨慎，> 45% 正常
  3. 基础过滤：TRADING 状态 + 永续 + USDT + 非稳定币 + 非黑名单 + 最低成交额
  4. 九维度评分 → 反转评分 >= effective_min_score（55 基础，cautious 模式 +15~20）
  5. 二次确认：距底 3-12%（必选）+ 四选二（KDJ / 1h RSI / 突破近高 / 回踩EMA不破）
  6. 去相关：排除与已选币种高度相关的候选
  7. LLM 评级 → Skill-3 策略 → Skill-4 执行

## 短期反转（4h K 线）— 日内/隔日波段
  适用场景：4h 级别右侧反转，捕捉动量确认后的波段机会
  K 线周期：4h（80 根 ≈ 13 天）
  核心信号：距底部理想距离 + KDJ 超卖金叉 + 前期跌幅深度
  持仓周期：4h ~ 1 天（max_hold_hours=24）

## 1h 反转（1h K 线）— 短线快进快出
  适用场景：1h 级别反转信号，小仓位 + 宽止损
  K 线周期：1h（80 根 ≈ 3 天）
  核心信号：距底部 + KDJ + MACD（三选二）
  持仓周期：1h ~ 12h
...
### 短期反转（4h）实际权重 v4（均衡调整）
  1. 距底部距离（25 分）— 右侧入场位置，距底部 3-12% 最佳
  2. KDJ 低位金叉（20 分）— 超卖区金叉，动量反转核心确认
  3. 底部放量（15 分）— 区分真假反转的关键信号，近期量能 vs 前期地量
  4. 前期跌幅深度（15 分）— 跌得越深反转空间越大
  5. 价格企稳（10 分）— 不再创新低 + 波动收窄，底部构筑确认
  6. 资金费率回归（8 分）— 从极端负值回归正常，空头平仓信号
  7. 长下影线（3 分）— 辅助信号，下方有强支撑
  8. 均线拐头（2 分）— 辅助信号，EMA5 上穿 EMA10
  9. MACD 反转信号（2 分）— 辅助信号，零轴下方金叉

### 1h 反转实际权重（回测调优后）
  1. 资金费率回归（20 分）— 1h 实盘最有效维度
  2. KDJ 低位金叉（15 分）— 唯一正向有效的动量指标
  3. 均线拐头（14 分）— 弱有效，升权后适度贡献
  4. 前期跌幅深度（12 分）— 深跌提供反转空间
  5. 底部放量（8 分）— 回测负效果，大幅降权
  6. 价格企稳（8 分）— 回测反效果，降权
  7. MACD 反转信号（8 分）— 弱有效
  8. 距底部距离（10 分）— 位置参考
  9. 长下影线（5 分）— 弱有效
...
### 长期反转（1d）权重分配
  1. 底部放量（15 分）— 日线放量更可靠
  2. 均线拐头（15 分）— 日线均线拐头信号强
  3. MACD 反转信号（15 分）— 日线 MACD 金叉/底背离可靠性高
  4. 价格企稳（12 分）— 日线级别企稳
  5. 资金费率回归（10 分）— 长期看权重降低
...
# 短期评分权重（满分 100）— 右侧确认 v4
# v3 回测（2025-11-01~2026-05-12）中放量/企稳反效果，但 v3 权重下这三个维度
# 几乎无筛选作用，导致假反转（无放量确认）也能拿高分。
# v4 均衡调整：降低位置+跌幅的绝对主导（78%→60%），提高放量和企稳权重。
# 需重新回测验证放量/企稳在新权重下是否转为正向。
ST_W_VOLUME_SURGE = 15
ST_W_PRICE_STABLE = 10
ST_W_MA_TURN = 2
ST_W_FUNDING = 8
