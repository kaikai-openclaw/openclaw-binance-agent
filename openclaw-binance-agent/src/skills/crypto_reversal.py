...
### 短期反转（4h）实际权重 v3（回测验证）
  回测验证（21149 样本）：距底部距离是唯一有效维度（+0.95%），
  放量（-0.66%）和企稳（-0.28%）均为反效果，不能提高权重。
  1. 距底部距离（35 分）— 唯一有效维度，距底部 3-12% 最佳
  2. KDJ 低位金叉（25 分）— 弱有效（+0.09%），超卖区金叉确认
  3. 前期跌幅深度（18 分）— 近中性（+0.01%），深跌提供反转空间
  4. 资金费率回归（8 分）— 空头平仓信号
  5. 底部放量（6 分）— 反效果（-0.66%），仅保留参考分
  6. 长下影线（2 分）— 弱有效（+0.04%）
  7. 价格企稳（2 分）— 反效果（-0.28%），仅保留参考分
  8. 均线拐头（2 分）— 反效果（-0.29%），仅保留参考分
  9. MACD 反转信号（2 分）— 反效果（-0.04%），仅保留参考分
...
# 回测验证（2025-11-01~2026-05-12，21149 样本）：
#   唯一有效：距底部距离（+0.95%）
#   弱有效：KDJ 金叉（+0.09%）、前期跌幅（+0.01%）、长下影线（+0.04%）
#   反效果：底部放量（-0.66%）、价格企稳（-0.28%）、均线拐头（-0.29%）、MACD（-0.04%）
# v4 均衡调整尝试提高放量/企稳权重，回测证实效果更差，回退到 v3。
ST_W_VOLUME_SURGE = 6
ST_W_PRICE_STABLE = 2
ST_W_MA_TURN = 2
ST_W_FUNDING = 8
