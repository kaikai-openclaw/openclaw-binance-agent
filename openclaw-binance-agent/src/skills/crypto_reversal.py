...
### 短期反转（4h）实际权重（回测调优后）
  1. 距底部距离（35 分）— 右侧入场位置，距底部 3-12% 最佳
  2. KDJ 低位金叉（25 分）— 超卖区金叉，动量反转核心确认
  3. 前期跌幅深度（18 分）— 跌得越深反转空间越大
  4. 资金费率回归（8 分）— 从极端负值回归正常，空头平仓信号
  5. 底部放量（6 分）— 辅助信号，近期量能 vs 前期地量
  6. 价格企稳（2 分）— 辅助信号，不再创新低 + 波动收窄
  7. 均线拐头（2 分）— 辅助信号，EMA5 上穿 EMA10
  8. MACD 反转信号（2 分）— 辅助信号，零轴下方金叉
  9. 长下影线（2 分）— 辅助信号，下方有强支撑
