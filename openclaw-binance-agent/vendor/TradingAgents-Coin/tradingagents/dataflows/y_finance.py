...
def get_stock_stats_indicators_window(
    ...
    if indicator not in best_ind_params:
        ...
        if key in aliases:
            indicator = aliases[key]
        else:
            # Fuzzy match: some models pass short names like "sma" instead of "close_50_sma"
            matched = [k for k in best_ind_params if key in k.lower()]
            if not matched:
                # Reverse substring: a supported name fragment appears in the input
                matched = [k for k in best_ind_params if k in key]
            if matched:
                indicator = matched[0]
            else:
                raise ValueError(
