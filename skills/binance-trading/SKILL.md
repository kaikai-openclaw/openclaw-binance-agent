...
user-invocable: true
metadata: {"openclaw":{"requires":{"env":["BINANCE_API_KEY","BINANCE_API_SECRET"],"bins":[".venv/bin/python3"]},"primaryEnv":"BINANCE_API_KEY"}}
---
...
```bash
.venv/bin/python3 {baseDir}/scripts/run_pipeline.py
.venv/bin/python3 {baseDir}/scripts/run_pipeline.py --paper
.venv/bin/python3 {baseDir}/scripts/run_pipeline.py --fast --symbols BTC,SOL
```
...
```bash
.venv/bin/python3 {baseDir}/scripts/check_account.py
```
...
```bash
.venv/bin/python3 {baseDir}/scripts/collect_candidates.py
.venv/bin/python3 {baseDir}/scripts/collect_candidates.py --symbols ONT,BTC
```
...
```bash
.venv/bin/python3 {baseDir}/scripts/analyze_symbol.py BTCUSDT
.venv/bin/python3 {baseDir}/scripts/analyze_symbol.py SOLUSDT --fast
```

## 运行测试

```bash
.venv/bin/python3 -m pytest {baseDir}/tests/ -q
```
