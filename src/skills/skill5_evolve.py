...
class Skill5Evolve(BaseSkill):
    ...
    def _lookup_strategy_tag(self, symbol: str) -> str:
        """
        从 state_store 的 skill4_execute 历史快照中查找 symbol 最近一次开仓的 strategy_tag。

        遍历所有快照（旧到新），后写覆盖前写，返回最新的 tag。
        找不到时返回空字符串。
        """
        if not hasattr(self, "_strategy_tag_cache"):
            self._strategy_tag_cache: dict[str, str] = {}
            try:
                conn = getattr(self.state_store, "_conn", None)
                if conn is not None:
                    rows = conn.execute(
                    ...
                    for (raw,) in rows:
                        ...
                        for e in data.get("execution_results", []):
                            ...
                            if sym and tag and tag not in ("unknown", "crypto_generic"):
                                self._strategy_tag_cache[sym] = tag
            except Exception as exc:
    ...
    def _record_closed_trades(
        ...
        for result in execution_results:
            ...
            close_reason = result.get("close_reason", "unknown")
            ...
            trade_record = TradeRecord(
                ...
                # strategy_tag 为空时从 state_store 历史记录查找，
                # 仍找不到时回退到 "crypto_generic"
                strategy_tag=(
                    strategy_tag
                    if strategy_tag and strategy_tag != "unknown"
                    else (self._lookup_strategy_tag(symbol) or "crypto_generic")
                ),
    ...
    def _sync_server_closed_trades(
        ...
        for result in execution_results:
            ...
            metadata_by_symbol[symbol] = {
                ...
                "strategy_tag": result.get("strategy_tag", "unknown"),
                "close_reason": result.get("close_reason", "server_close"),
            }
        ...
        for pos in account.positions or []:
            ...
            if symbol:
                symbols.add(symbol)
                # 持仓 symbol 不在本轮 execution_results 中时，
                # 从 state_store 历史记录补充 metadata，确保平仓同步时 strategy_tag 正确
                if symbol not in metadata_by_symbol:
                    looked_up_tag = self._lookup_strategy_tag(symbol)
                    if looked_up_tag:
                        metadata_by_symbol[symbol] = {
                            "rating_score": 6,
                            "position_size_pct": 0.0,
                            "hold_duration_hours": 0.0,
                            "strategy_tag": looked_up_tag,
